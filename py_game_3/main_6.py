...
def aplicar_gravedad(pantalla, personaje_animacion, rectangulo_personaje: pygame.Rect, pisos: list):
    ...
    if esta_saltando:
        animar_personaje(pantalla, personaje_animacion, rectangulo_personaje)
    ...
    for plataforma in pisos:
        if rectangulo_personaje.colliderect(piso):
            esta_saltando = False
            desplazamiento_y = 0 
            rectangulo_personaje.bottom = plataforma.top
            break
        else:
            esta_saltando = True
...
while True:
    ...
    actualizar_pantalla(PANTALLA, que_hace, rectangulo_personaje, velocidad, lista_plataforma)
